...
CSV2015 = "hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/ioanpapadopoulos/data/csv/yellow_2015_csv"
...
if __name__ == "__main__":
    ...
    df.createOrReplaceTempView("t2015")

    query = f"""
    WITH base AS (
      SELECT
        hour(to_timestamp(regexp_replace(substr(tpep_pickup_datetime,1,19),'T',' '), 'yyyy-MM-dd HH:mm:ss')) AS pickup_hour,
        (unix_timestamp(to_timestamp(regexp_replace(substr(tpep_dropoff_datetime,1,19),'T',' '), 'yyyy-MM-dd HH:mm:ss')) -
        unix_timestamp(to_timestamp(regexp_replace(substr(tpep_pickup_datetime,1,19),'T',' '), 'yyyy-MM-dd HH:mm:ss'))) / 60.0 AS duration_min,
